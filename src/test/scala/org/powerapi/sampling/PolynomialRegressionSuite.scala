...
class PolynomialRegressionSuite(system: ActorSystem) extends UnitTest(system) {
  ...
  trait Formulae {
    ...
    formulae :+= "  { coefficient = 13.0, formula = [87.51910110023778,8.72815576523707E-9,-1.2148402282161684E-18] }"
    formulae :+= "  { coefficient = 14.0, formula = [86.58573767540133,9.93246881358927E-9,-1.3973062582274987E-18] }"
    ...
    formulae :+= "  { coefficient = 19.0, formula = [88.9350908040251,8.944423260826779E-9,-8.127878568268322E-19] }"
  ...
  "The PolynomialRegression object" should "process the data files from a directory and write the formulae inside a result file" in new Formulae {
    ...
    PolynomialRegression(s"${basepath}pr-data", "/tmp/formulae", 2)
