...
case class LineMock(aggPowerReport: AggPowerReport) {
  override def toString() =
    ...
    "process=" + aggPowerReport.target + ";" +
    "device=" + aggPowerReport.device + ";" +
    "power=" + aggPowerReport.power
}
...
class ConsoleReporterSuite(system: ActorSystem) extends UnitTest(system) {
  ...
  "A console reporter" should "process a PowerReport and then report energy information in a String format" in {
    ...
    publishAggPowerReport(muid, target, 1.0, PowerUnit.W, device, tickMock)(eventBus)
    publishAggPowerReport(muid, target, 2.0, PowerUnit.W, device, tickMock)(eventBus)
    publishAggPowerReport(muid, target, 3.0, PowerUnit.W, device, tickMock)(eventBus)
