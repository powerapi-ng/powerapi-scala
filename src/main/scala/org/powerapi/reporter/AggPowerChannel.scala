...
object AggPowerChannel extends Channel {
   ...
   */
  def publishAggPowerReport(muid: UUID, target: Target, power: Double, unit: PowerUnit, device: String, tick: ClockTick): MessageBus => Unit = {
    publish(AggPowerReport(aggPowerReportTopic(muid), muid, target, power, unit, device, tick))
  }
