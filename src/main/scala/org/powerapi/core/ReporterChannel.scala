...
object ReporterChannel extends Channel {
   ...
   */
  case class ReporterStart(topic: String, muid: UUID, nbTarget: Int, aggFunction: List[PowerReport] => PowerReport) extends ReporterMessage
   ...
   */
  def startReporter(muid: UUID, nbTarget: Int, aggFunction: List[PowerReport] => PowerReport): MessageBus => Unit = {
