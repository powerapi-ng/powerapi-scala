...
import org.powerapi.module.libpfm.{PMU, LibpfmHelper, LibpfmCoreSensorModule}
...
object Application extends Configuration(None) with App {
  ...
  for(workload <- workloads) {
    for(benchmark <- workload.benchmarks) {
      ...
      for((subset, index) <- events.zipWithIndex) {
        ...
        for(pmu <- pmus) Path(s"phase1/$benchmark/subset-$index/${pmu.name}", '/').createDirectory()
        (Path(".", '/') * ((p: Path) => !p.name.equals(outputPowers) && p.name.endsWith(".dat"))).foreach(path => {
          ...
          path.moveTo(Path(s"phase1/$benchmark/subset-$index/$pmu/${path.name}", '/'), true)
        })
        Path(outputPowers, '/').moveTo(Path(s"phase1/$benchmark/subset-$index/$outputPowers", '/'), true)
  ...
  for(benchmarkPath <- Path(s"phase1", '/') ** IsDirectory) {
    for(subsetPath <- benchmarkPath ** IsDirectory) {
  ...
  for(workload <- workloads) {
    for(benchmark <- workload.benchmarks) {
      ...
      for((subset, index) <- events.zipWithIndex) {
        ...
        for(pmu <- pmus) Path(s"phase2/$benchmark/subset-$index/${pmu.name}", '/').createDirectory()
        (Path(".", '/') * ((p: Path) => !p.name.equals(outputPowers) && p.name.endsWith(".dat"))).foreach(path => {
          val pmu = path.name.substring(0, path.name.indexOf("--")).replace("output-", "")
          path.moveTo(Path(s"phase2/$benchmark/subset-$index/$pmu/${path.name}", '/'), true)
        })
        Path(outputPowers, '/').moveTo(Path(s"phase2/$benchmark/subset-$index/$outputPowers", '/'), true)
