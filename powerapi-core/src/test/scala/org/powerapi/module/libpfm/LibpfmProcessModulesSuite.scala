...
import org.powerapi.core.target.{Process, Application, TargetUsageRatio}
import scala.collection.BitSet
import scala.concurrent.duration.DurationInt

class LibpfmProcessModulesSuite(system: ActorSystem) extends UnitTest(system) {

  implicit val timeout = Timeout(1.seconds)

  def this() = this(ActorSystem("LibpfmProcessModulesSuite"))

  override def afterAll() = {
    TestKit.shutdownActorSystem(system)
  }

  "The LibpfmProcessModule class" should "create the underlying classes (sensors/formulae)" in {
