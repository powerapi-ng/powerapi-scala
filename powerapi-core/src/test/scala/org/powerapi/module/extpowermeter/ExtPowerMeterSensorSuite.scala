...
import org.powerapi.core.target.{All, Application, Target, TargetUsageRatio}
...
class MockExtPowerMeterSensor(eventBus: MessageBus, muid: UUID, target: Target, osHelper: OSHelper, pMeter: ExternalPMeter, idlePower: Power)
  extends ExtPowerMeterSensor(eventBus, muid, target,
    MockChannel.subscribeRawReportMock, MockChannel.unsubscribeRawReportMock, MockChannel.publishReportMock,
    osHelper, pMeter, idlePower)

class ExtPowerMeterSensorSuite extends UnitTest with MockFactory {
  ...
  "An ExtPowerMeterSensor" should "handle MonitorTick and RawPowerReport messages, and sense cpu metrics for the All target" in new Bus {
    ...
    EventFilter.info(occurrences = 1, start = s"sensor is started, class: ${classOf[MockExtPowerMeterSensor].getName}").intercept({
      startSensor(muid, target, classOf[MockExtPowerMeterSensor], Seq(eventBus, muid, target, osHelper, extPMeter, 90.W))(eventBus)
    })
    MockChannel.subscribeReportMock(muid, target)(eventBus)(testActor)

    MockChannel.publishRawReportMock(105.W)(eventBus)
    ...
    reportMock.source should equal("extmock")

    MockChannel.publishRawReportMock(115.W)(eventBus)
    ...
    EventFilter.info(occurrences = 1, start = s"sensor is stopped, class: ${classOf[MockExtPowerMeterSensor].getName}").intercept({
  ...
  it should "handle MonitorTick and RawPowerReport messages, and sense cpu metrics for the other targets" in new Bus {
    val osHelper = mock[OSHelper]
    val extPMeter = mock[ExternalPMeter]
    ...
    publishMonitorTick(muid, target, tick3)(eventBus)
    MockChannel.publishRawReportMock(125.W)(eventBus)
    expectNoMsg()
