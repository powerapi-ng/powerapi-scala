...
import org.powerapi.module.extpowermeter.ExtPowerMeterChannel.publishRAPLPowerReport

class RAPLFormulaSuite extends UnitTest {
  ...
  "A RAPLFormula" should "process a SensorReport and then publish a RawPowerReport" in new Bus {
    ...
    EventFilter.info(occurrences = 1, start = s"formula is started, class: ${classOf[RAPLFormula].getName}").intercept({
      startFormula(muid, target, classOf[RAPLFormula], Seq(eventBus, muid, target))(eventBus)
    })
    subscribeRawPowerReport(muid)(eventBus)(testActor)

    publishRAPLPowerReport(muid, target, targetRatio, 25.W, tick1)(eventBus)
    ...
    rawPowerReport.device should equal("rapl")
    ...
    publishRAPLPowerReport(muid, target, targetRatio, 30.W, tick2)(eventBus)
    expectNoMsg()
