...
import org.powerapi.module.extpowermeter.ExtPowerMeterChannel.publishG5KOmegaWattPowerReport

class G5kOmegaWattFormulaSuite extends UnitTest {
  ...
  "A G5kOmegaWattFormula" should "process a SensorReport and then publish a RawPowerReport" in new Bus {
    ...
    EventFilter.info(occurrences = 1, start = s"formula is started, class: ${classOf[G5kOmegaWattFormula].getName}").intercept({
      startFormula(muid, target, classOf[G5kOmegaWattFormula], Seq(eventBus, muid, target))(eventBus)
    })
    subscribeRawPowerReport(muid)(eventBus)(testActor)

    publishG5KOmegaWattPowerReport(muid, target, targetRatio, 25.W, tick1)(eventBus)
    ...
    rawPowerReport.device should equal("g5k-omegawatt")
    rawPowerReport.tick should equal(tick1)

    EventFilter.info(occurrences = 1, start = s"formula is stopped, class: ${classOf[G5kOmegaWattFormula].getName}").intercept({
      stopFormula(muid)(eventBus)
    })

    publishG5KOmegaWattPowerReport(muid, target, targetRatio, 30.W, tick2)(eventBus)
    expectNoMsg()
