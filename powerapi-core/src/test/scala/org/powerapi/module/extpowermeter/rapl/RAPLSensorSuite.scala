...
import org.powerapi.module.extpowermeter.ExtPowerMeterChannel.{ExtPowerMeterPowerReport, publishRAPLRawPowerReport, subscribeRAPLPowerReport}
...
class RAPLSensorSuite extends UnitTest with MockFactory {
  ...
  "A RAPLSensor" should "handle MonitorTick and RawPowerReport messages for the All target" in new Bus {
    ...
    EventFilter.info(occurrences = 1, start = s"sensor is started, class: ${classOf[RAPLSensor].getName}").intercept({
      startSensor(muid, target, classOf[RAPLSensor], Seq(eventBus, muid, target, osHelper, extPMeter))(eventBus)
    })
    subscribeRAPLPowerReport(muid, target)(eventBus)(testActor)

    publishRAPLRawPowerReport(25.W)(eventBus)
    ...
    reportMock.power should equal(25.W)
    reportMock.source should equal("rapl")

    publishRAPLRawPowerReport(15.W)(eventBus)
    ...
    reportMock.targetRatio should equal(TargetUsageRatio(1))
    reportMock.power should equal(15.W)
    reportMock.source should equal("rapl")
  ...
  it should "handle MonitorTick and RawPowerReport messages for the other targets" in new Bus {
    ...
    publishRAPLRawPowerReport(20.W)(eventBus)
    publishMonitorTick(muid, target, tick2)(eventBus)
    reportMock = expectMsgClass(classOf[ExtPowerMeterPowerReport])
    reportMock.muid should equal(muid)
    reportMock.target should equal(target)
    ...
    reportMock.power should equal(20.W)
    reportMock.source should equal("rapl")
