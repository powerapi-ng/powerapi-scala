...
/**
  ...
  */
class HWCCoreFormulaConfiguration(prefix: Option[String]) extends Configuration(prefix) {

  lazy val formulae: Map[Double, List[Double]] = load { conf =>
    (for (item: Config <- conf.getConfigList(s"${configurationPath}powerapi.hwc.formulae.cycles").asScala)
      yield (item.getDouble("coefficient"), item.getDoubleList("formula").asScala.map(_.toDouble).toList)).toMap
  } match {
