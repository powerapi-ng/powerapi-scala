...
import ancillary.AncillaryLibrary
...
import org.powerapi.module.libpfm.{AncillaryHelper, LibpfmModule, LibpfmHelper, LibpfmCoreProcessModule, LibpfmCoreModule, LibpfmProcessModule, LibpfmCoreCodeModule}
...
object PowerAPI extends App {
  val modulesR = """(procfs-cpu-simple|sigar-cpu-simple|cpu-dvfs|libpfm|libpfm-process|libpfm-core|libpfm-core-process|libpfm-core-code|powerspy|g5k-omegawatt|rapl)(,(procfs-cpu-simple|sigar-cpu-simple|cpu-dvfs|libpfm|libpfm-process|libpfm-core|libpfm-core-process|libpfm-core-code|powerspy|g5k-omegawatt|rapl))*""".r
  ...
  def printHelp(): Unit = {
    val str =
      """
        ...
        |
        |usage: ./powerapi modules procfs-cpu-simple|sigar-cpu-simple|cpu-dvfs|libpfm|libpfm-process|libpfm-core|libpfm-core-process|libpfm-core-code|powerspy|g5k-omegawatt|rapl (1, *) *--prefix [name]*
        |                          monitor (1, *)
        |                            --frequency $MILLISECONDS
        |                            --self (0, 1) --pids [pid, ...] (0, *) --apps [app, ...] (0, *) --codes [label, ...] (0, *) | all (0, 1)
  ...
  def cliMonitorsSubcommand(options: List[Map[Symbol, Any]], currentMonitor: Map[Symbol, Any], args: List[String]): (List[String], List[Map[Symbol, Any]]) = args match {
    ...
    case "--codes" :: value :: tail if validate(codesR, value) => cliMonitorsSubcommand(options, currentMonitor + ('targets -> (currentMonitor.getOrElse('targets, Set[Any]()).asInstanceOf[Set[Any]] ++ value.split(",").map(code => Code(code)))), tail)
    ...
    for(powerMeterConf <- configuration) {
      val modules = (for(module <- powerMeterConf('modules).toString.split(",")) yield {
        module match {
          ...
          case "libpfm-core-code" => LibpfmCoreCodeModule(powerMeterConf('prefix).asInstanceOf[Option[String]], libpfmHelper.get, new AncillaryHelper)
