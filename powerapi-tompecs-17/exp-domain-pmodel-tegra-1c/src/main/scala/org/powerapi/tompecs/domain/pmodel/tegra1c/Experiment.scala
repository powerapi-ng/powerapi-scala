 ...
 */
package org.powerapi.tompecs.domain.pmodel.tegra1c
...
object Experiment extends Configuration(None) with App {
  ...
  val pOutput = new FileDisplay("/tmp/data/output-powerapi-cg.dat")
  ...
  val pidsBench = for(pid <- output) yield {
    pid match {
  ...
  val wload = powerapi.monitor(interval)(pidsBench: _*)(MEAN).to(pOutput)
